...
local function fetchJSON(url)
    -- GETDocument parses its response as HTML. That is useful for websites,
    -- but destructive for a JSON field which itself contains HTML: embedded
    -- <p> and <br> tags are stripped before decodeJSON sees them. Read the
    -- response body directly so WordPress's chapter markup stays intact.
    local response = Request(GET(url))
    local status = response:code()
    if status < 200 or status >= 300 then
        error("Gold Silver Translation: WordPress API returned HTTP " .. tostring(status))
    end

    local body = response:body()
    if not body then
        error("Gold Silver Translation: WordPress API returned an empty response")
    end

    local raw = body:string()
...
local function fetchChapterData()
    ...
    while page <= 50 do
        local data = fetchJSON(apiURL(page))
...
local function parsePassage(chapterURL)
    ...
    local data = fetchJSON(apiBase .. "slug:" .. slug .. "?fields=content")
